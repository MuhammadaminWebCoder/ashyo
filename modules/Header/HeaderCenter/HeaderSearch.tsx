...
const HeaderSearch:FC<{isMobile?:boolean}> = ({isMobile}) => {
   ...
   return (
    <div className={`flex gap-[5px] !mt-5 min-[1131px]:!mt-0 sm:gap-[10px] ${isMobile ? "hidden-searchbar" :""}`}>
        ...
        <div className={`relative ${!isMobile && "w-[65%] sm:w-full"}`}>
            <Input value={value} onChange={(e) => handleSearch(e.target.value)} extraClass={`${!isMobile && "!w-full"} sm:!h-[48px] !h-[40px]`} placeholder={t("inputPlaceholder")} type='text' name='search'/>
