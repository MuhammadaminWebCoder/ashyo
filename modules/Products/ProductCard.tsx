...
const ProductCard: FC<{ item: ProductType }> = ({ item }) => {
  ...
  return (
    <div id='products_card' onClick={(e) => (e.target as HTMLDivElement).id == "products_card" &&  router.push(`/products/${item.id}`)} className='max-w-[273px]'>
      <div className='bg-[#EBEFF3] cursor-pointer rounded-[8px] mb-4 relative h-[180px] sm:h-[280px] flex items-center justify-center'>
        <Image id='products_card' className='w-[130px] sm:w-[202px] h-[130px] sm:h-[202px]' src={`${IMG_API}/${item.image}`} alt='Product img' width={202} height={202} priority />
        <button className='absolute top-[12px] sm:top-[20px] right-[12px] sm:right-[24px] cursor-pointer'> <LikeIcon/> </button>
      </div>
      <p id='products_card' className='text-[#545D6A] line-clamp-1 sm:line-clamp-none text-[12px] sm:text-[14px] mb-[20px] sm:mb-[28px]'>{item.description}</p>
      <div className='flex flex-col sm:flex-row w-full justify-between items-start sm:items-center'>
        <strong id='products_card' className='text-[#0A1729] text-[15px] sm:text-[20px] mb-[10px]'>{formatNumber(item.price)} usz</strong>
